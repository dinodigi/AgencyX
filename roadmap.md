...
2. Clerk login: **real Clerk sign-in BUILT** in the Electron renderer (`renderer/ClerkAuth.tsx` — ClerkProvider + SignIn + org picker; `ClerkTokenBridge` pushes+refreshes the token to main every 45s). `main/auth.ts` is now a mirror of the renderer's auth (no refresh loop). Dev paste-form kept as fallback when no publishable key. **TODO: serve packaged renderer over loopback (Clerk cookie session fails on file://).**
